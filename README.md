...
数据流：
ESP32 → MQTT Broker → 后端（Flask + MQTT Client）→ WebSocket 推送 → 浏览器看板  
后端会将产量、温度、状态写入 SQLite。

## 项目功能

- 按键计数：短按按钮模拟生产一次，产量 +1，并上报 MQTT
- 温度模拟：电位器模拟温度，ESP32 定时上报温度
- 故障模拟：长按按钮触发故障状态（fault），网页端报警闪烁
- 过热提示：温度超过阈值显示 OVERHEAT
- 网页实时看板：
  - 显示产量、温度、OEE（演示用）、设备状态
  - ECharts 折线图实时显示产量趋势（最近 20 个点）
- 数据持久化：后端使用 SQLite 保存日志；页面刷新后通过 `/api/history` 恢复累计产量
